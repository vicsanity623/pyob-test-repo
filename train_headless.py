    ...
    def forward_plan(self, env_copy_func, steps=PLANNING_HORIZON):
        ...
        for action_variation in np.linspace(-0.5, 0.5, 3):
            ...
            for step in range(steps):
                ...
                # Corrected reward and derivative calculation, incorporating motor feedback
                total_reward += (
                    np.sum(outputs) * 0.1 + np.sum(motor * np.array([1.0, 1.0])) * 0.5
                )
                sim_derivative = (
                    -sim_voltages
                    + (
                        np.dot(self.weights, outputs)
                        + self.biases
                        + (motor * 10.0 if self.size >= 2 else np.zeros(self.size))
                    )
                ) / self.time_constants
